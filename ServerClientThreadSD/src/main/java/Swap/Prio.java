    ...
    public static void main(String[] args)
    {
        ...
        while(N != 0)
        {
            ...
            for (int i = 0; i < N; i++)
            {
                ...
                System.out.println("Tamanho do processo P" + (i+1) + ":");
            ...
            while(qteprocessos > 0)
            {
                ...
                for (int i = 0; i < N; i++)
                {
                    if ((int) ingressos.get(i) != -1 && (int) ingressos.get(i) <= tempoIng)
                        processos.add(i); // Adiciona na lista de processos
                }
            ...
            /* RASTREAMENTO DE CÓDIGO -> REMOVER ISSO MAIS TARDE
            System.out.println(cpentradas.toString());
            System.exit(0);
            */
            
            int[] mp = new int[5];  // memória principal
            int[] sp = new int[10]; // swap
            ...
            while(counter < sp.length) 
            {
                ...
                {
                    mp[i] = (int) cpentradas.get(i);
                }
                if(mp.length == i) 
                {
                    sp[j] = (int) cpentradas.get(i);
                    j++; 
                }
                i++;
                counter++;
            ...
            {
                System.out.printf("%d\t", mp[k]);
            }
            System.out.println("\nSWAP");
            for(int k = 0; k < sp.length; k++)
            {
                System.out.printf("%d\t", sp[k]);
            }
            

            System.out.println("\n\nDigite qualquer valor para continuar ou 0 para finalizar:");
            N = scan.nextInt();
        }
